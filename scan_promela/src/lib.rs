...
pub fn load(
    ...
    if path.is_file() {
        ...
        bail!("{} is not a valid file or directory", path.display())
    }
...
fn process_file(path: &Path, debug_mode: bool) -> anyhow::Result<(PromelaScan, PromelaModel)> {
    ...
    match path.extension().and_then(|e| e.to_str()) {
        Some("pml" | "prm") => {}
        Some(ext) => {
            bail!("“.{ext}” is not a valid Promela extension.");
        }
        None => {
            bail!("file has no extension (expected .pml / .prm).");
        }
    }

    /* ---------- 2. read file ------------------------------------------------ */
    let raw = read_file(path)
        .with_context(|| format!("failed reading file {}", path.to_str().unwrap()))?;
    ...
    let ast = if debug_mode {
        ...
        parse_content(&expanded).context("failed parsing file content")?
    };
...
fn read_file(file_path: &Path) -> anyhow::Result<String> {
    let mut file =
        File::open(file_path).with_context(|| anyhow!("failed opening file {:?}", file_path))?;

    let mut content = String::new();
    file.read_to_string(&mut content)
        .with_context(|| anyhow!("failed reading file {:?}", file_path))?;
...
fn parse_content(content: &str) -> anyhow::Result<Vec<Module>> {
    // ------------------------------------------------------------- Lexing
    ...
    */
    ...
    // -------------------------------------------------------- Return AST or error
    let result = result.ok_or(anyhow!("Parsing failed: no result produced."))?;
    result
        .map_err(|err| anyhow!(err.as_ref().to_string()))
        .context("Parser returned an error")
}
...
fn replace_macros_in_content(src: &str) -> String {
    ...
    for (k, v) in &constants {
        info!("Collected #define constant: {k} -> {v}");
    }
